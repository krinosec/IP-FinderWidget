...
/* *******************************************************************
   Merged Implementation for GNOME 45+ with async/await,
   enhanced error handling, connectivity monitoring, custom API support,
   and updated UI elements.
******************************************************************* */
...
class IPFinderWidget extends PanelMenu.Button {
    ...
    _buildUI() {
        // Build the panel button UI with a globe icon and an informational label
    ...
    async _fetchIPData(ip) {
        ...
        switch(apiService) {
            ...
            case API_SERVICES.CUSTOM:
                // Replace %s in the URL with the IP (or empty string)
    ...
    _updateDisplay(data) {
        ...
        // Remove any previously added info items
    ...
    _checkVPNStatus(data) {
        ...
        // Check if any VPN keywords appear in data.security or data.network (if available)
