...
// Constant for the custom API option (must match the value in prefs.js)
const CUSTOM_API = 2;
...
var IPFinderMenuButton = GObject.registerClass(
  class IPFinderMenuButton extends PanelMenu.Button {
    ...
    async _syncConnectivity() {
      ...
      if (this._mainConnection == null || this._mainConnection.state !== NM.ActiveConnectionState.ACTIVATED) {
        this._setIpDetails();
      ...
      this._portalHelperProxy.call(
        'Open',
        new GLib.Variant('(ss)', [path, timestamp.toString()]),
        ...
        (proxy, res) => {
          try {
            ...
            debugLog(`Error in portal helper Call: ${e}`);
          }
        }
      );
    ...
    async _getIpInfo() {
      ...
      // Save current API selection for display processing.
    ...
    _setIpDetails(data, error) {
      // Clear children from the info and map boxes.
      ...
      this._vpnInfoBox.setVpnStatus({
        ...
        vpnName: this._vpnConnectionName ? this._vpnConnectionName : _('Off'),
